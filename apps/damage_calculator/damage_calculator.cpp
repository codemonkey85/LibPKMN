...
int main(int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("gen", po::value<unsigned int>(&gen)->default_value(5), "Specify a generation (1-5).")
    ;
    po::variables_map vm;
    ...
    vector<unsigned int> damage_range, stat_range;
    ...
    get_damage_range(attacker, defender, attack, attacker_level, defender_level, damage_range);

    //Getting proper stat strings
    if(attack->get_move_damage_class() == Move_Classes::PHYSICAL) damage_class_string = "Physical";
    else if(attack->get_move_damage_class() == Move_Classes::SPECIAL) damage_class_string = "Special";
    else damage_class_string = "Non-damaging";

    //Get type mod
    type_mod = get_type_damage_mod(attack->get_type(), defender->get_types()[0], (gen != 1));
    type_mod *= get_type_damage_mod(attack->get_type(), defender->get_types()[1], (gen != 1));

    if(attack->get_move_damage_class() != Move_Classes::NON_DAMAGING)
    {
    ...
    if(attack->get_move_damage_class() != Move_Classes::NON_DAMAGING and type_mod != 1.0) cout << endl << type_mod_string << endl;
