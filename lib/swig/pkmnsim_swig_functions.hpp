...
namespace pkmnsim
{
    ...
    b_pkmn_vec_t get_pokemon_vec_of_type(std::string type1, std::string type2,
                                         unsigned int gen, bool lax);
    
    //pkmnsim/analysis/stats.hpp
    std::vector<unsigned int> get_stat_range_vec(base_pokemon::sptr b_pkmn,
                                                 unsigned int stat,
                                                 unsigned int level,
                                                 unsigned int gen);

    //pkmnsim/analysis/damage.hpp
    std::vector<unsigned int> get_damage_range_vec(base_pokemon::sptr attacker,
                                                   base_pokemon::sptr defender,
                                                   move::sptr attack,
                                                   unsigned int attacker_level,
                                                   unsigned int defender_level);
    std::vector<unsigned int> get_damage_range_vec(team_pokemon::sptr attacker,
                                                   team_pokemon::sptr defender,
                                                   move::sptr attack);
