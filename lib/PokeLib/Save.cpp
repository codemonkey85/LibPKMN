...
uint32_t getBoxOffset(int box, SaveType t) {
	if(box < 1 || box > 18) return 0;
	if(t == HGSS) return (30*(box-1)*0x88+(box-1)*16);
...
void Save::setSavePosition(SavePosition p) {
	if(getSaveFormat() == RAW256KiB) return;
	//BlockA (small block)
	bool sub = false;
	if(p == SaveSub) sub = true;
...
Box *Save::getBox(uint8_t n) const {
	...
	if(getSaveType() == HGSS) offset += 16+8;
	//Each box name is 22 bytes long (11 uint16 characters)
	b->setName(PokeText::convertFrom(PokeText::toPoketext((uint16_t*)&BlockB[offset],11)));
