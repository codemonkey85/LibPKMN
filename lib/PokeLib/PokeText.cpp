...
const uint16_t poketext_0x0400[] = {
...
widetext PokeText::convertFrom(const poketext &text) {
	widetext output;
	for(uint8_t i=0; i<text.length(); ++i) {
		...
		if(text[i] >= 0x400) {
			output += (wchar_t)poketext_0x0400[text[i]];
		} else {
			output += (wchar_t)poketext_0x0000[text[i]];
		}
	}
	return output;
}
poketext PokeText::convertTo(const widetext &text) {
	poketext output;
	for(uint16_t i=0; i<text.length(); ++i) {
		...
		for(uint16_t j=0; j < 485; ++j) {
			if(text[i] == poketext_0x0000[j]) {
				output += j;
				break;
			}
		}
...
poketext PokeText::toPoketext(const uint16_t* arr, size_t length) {
	poketext output;
...
widetext PokeText::toWidetext(const wchar_t* wc, size_t length) {
	widetext output;
	for(uint16_t i=0; i<length; ++i) {
		output += wc[i];
	}
